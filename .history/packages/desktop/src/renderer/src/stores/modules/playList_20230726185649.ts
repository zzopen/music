...
export const usePlayListStore = defineStore({
  ...
  actions: {
    ...
    async getSongSheetSongList(songsheetId: number | string) {
      this.reset()

      const cover = new URL(`../../assets/86.jpg`, import.meta.url).href
      let songList = [{ name: '123', src: '', singer: '', cover: '', album: '' }] //await songsheet.songListAction({ songsheetId })
      songList = songList.map((item) => {
        const src = new URL(item.src, import.meta.url).href
        console.log('src:', src)
        item.src = 'http://localhost:8988/Avid.mp3'
