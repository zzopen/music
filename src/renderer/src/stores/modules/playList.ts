  ...
  actions: {
    ...
    async __test__setPlayList() {
      ...
      for (const name of songNames) {
        const { default: lyric } = await import(
          `../../assets/songs/${name}.lrc?raw`
        );

        const src = new URL(`../../assets/songs/${name}.mp3`, import.meta.url)
          .href;
    ...
    getLastSong(): Song {
      const index =
        this.curPlayListPosition - 1 < 0
          ? this.playList.length - 1
          : this.curPlayListPosition - 1;
    ...
    getNextSong(): Song {
      const index =
        this.curPlayListPosition + 1 > this.playList.length - 1
          ? 0
          : this.curPlayListPosition + 1;
      if (this.playList[index]) {
        this.curPlayListPosition = index;
