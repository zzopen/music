...
import type { Song, Lyrics } from "@/business/typings";
import { PlayerStatus } from "@/business/typings";
import { getCurLyricIndex, omit, consoleLog, delay } from "@/business/utils";
...
export const usePlayerStore = defineStore({
  ...
  getters: {
    ...
    getLyricList(state): Lyrics {
      if (!state.song || !state.song.lyricsText) {
        return [];
      }
  ...
  actions: {
    ...
    /**** player ****/
    _setSong(song: Nullable<Song>) {
      this.reset();
      this.song = song;
    },
    changeSong(song: Nullable<Song>) {
      if (!song) {
        return;
      }
    ...
    cachePlayProgress() {
      this.playProgress = this.totalDuration
        ? (this.curTime / this.totalDuration) * 100
        : 0;
    },
    /**** reset ****/
    reset() {
      this.$patch((state) => {
        ...
        state.song = defaultPlayerState.song
